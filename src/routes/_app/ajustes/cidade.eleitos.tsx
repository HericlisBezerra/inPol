...
export const Route = createFileRoute("/_app/ajustes/cidade/eleitos")({
...
/**
 * Ajustes · Como a IA entende minha cidade · Eleitos (TSE) — antes /ajustes/eleitos.
 *
 * Ficou junto do Vocabulário porque é a mesma intenção: ensinar a IA a reconhecer
 * gente da cidade. Importar do TSE joga os nomes no vocabulário; o alinhamento muda
 * como a IA lê as menções. Dados reais via elected.functions.ts.
 */
function Screen() {
  ...
  return (
    <div>
      <PanelHeader
        title="Eleitos (TSE)"
        description="Vereadores importados dos dados abertos do TSE."
        effect={
          <>
            Importar joga todos os nomes no Vocabulário de uma vez — a IA passa a reconhecê-los nos
            grupos e na imprensa. O <b className="text-v2-ink">alinhamento</b> é o ponto de partida
            da leitura política de cada menção; vira automático depois de 10 votações registradas.
          </>
        }
        action={
          <div className="relative">
            ...
            {importOpen && (
              <div className="absolute right-0 top-11 z-50 w-72 rounded-xl border border-v2-line bg-v2-surface p-3.5 shadow-[0_16px_48px_rgba(33,31,28,0.16)]">
                ...
                <FieldLabel className="mt-2.5">Município</FieldLabel>
                <select
                  value={muniCode}
                ...
                <FieldHint>
                  Traz os eleitos do município e adiciona todos ao Vocabulário como pessoas
                  reconhecíveis. Todos entram como <b>Indep.</b> até você classificar.
                </FieldHint>
                {importMut.isError && (
      ...
      <div className="mt-3 overflow-hidden rounded-[13px] border border-v2-line bg-v2-card">
        ...
        {!isLoading && vereadores.length === 0 && (
          <div className="px-5 py-4 text-[12.5px] text-v2-faint">
            Nenhum vereador importado ainda — a IA não reconhece os nomes da Câmara nas mensagens.
          </div>
        )}
        {visible.map((v, i) => (
          <VereadorRow
      ...
      <FieldHint>
        Base / Indep. / Oposição muda como a IA interpreta as citações da pessoa e como ela é
        agrupada nos relatórios. O × remove o eleito do Vocabulário — a IA para de reconhecer o
        nome.
      </FieldHint>
    ...
    >
      ...
      <div className="flex items-center gap-1">
        ...
        <button
          onClick={onRemove}
          disabled={removing}
          aria-label={`Remover ${name} do Vocabulário`}

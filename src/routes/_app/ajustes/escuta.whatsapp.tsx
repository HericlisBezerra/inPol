...
export const Route = createFileRoute("/_app/ajustes/escuta/whatsapp")({
  head: () => ({ meta: [{ title: "Grupos de WhatsApp — Ajustes" }] }),
  component: WhatsApp,
});

/**
 * Ajustes · O que eu escuto · Grupos de WhatsApp (antes /ajustes/whatsapp).
 ...
 *
 * O toggle por grupo é a chave de ingestão: enquanto estiver desligado, as mensagens
 * daquele grupo nunca chegam ao banco. Isso agora está escrito na tela, não implícito.
 */
  ...
  return (
    <div>
      <PanelHeader
        ...
        description="Cada instância é um número que participa dos grupos monitorados."
        effect={
          <>
            Só os grupos com o monitoramento <b className="text-v2-ink">ligado</b> têm mensagens
            coletadas. Grupo desligado não gera sinal, não vira alerta e não é citado em relatório —
            nem retroativamente.
          </>
        }
        action={
          <button className="rounded-lg bg-v2-ink px-3.5 py-2 text-[13px] font-[650] text-v2-card">
            ＋ Nova instância
          </button>
        }
      />
      ...
      {!loadingInstances && !errorInstances && instances.length === 0 && (
        <div className="mt-4 rounded-[13px] border border-dashed border-v2-line bg-v2-card px-5 py-9 text-center text-[13px] text-v2-ink-3">
          ...
          <FieldHint>
            Sem instância conectada, o inPol não recebe nenhuma mensagem de WhatsApp — as telas de
            Sinais e Alertas ficam alimentadas só pela imprensa.
          </FieldHint>
        </div>
      )}
      ...
      <Consequence icon="🛡">
        ...
        são coletadas — política LGPD aplicada na ingestão. Todo acesso ao conteúdo bruto dessas
        mensagens é registrado na trilha de auditoria.
      </Consequence>
    </div>
  );
}
...
function InstanceCard({
  ...
  return (
    <div className="rounded-[13px] border border-v2-line bg-v2-card px-5 py-[18px]">
      ...
      {/* Instância desconectada é a falha silenciosa mais cara aqui: a tela continua
          listando grupos "monitorados" enquanto nada entra. Dizer isso na cara. */}
      {meta.tone === "crit" && (
        <div className="mt-3 rounded-lg border border-v2-crit bg-v2-crit-bg px-3 py-2 text-[12px] leading-snug text-v2-crit">
          Instância desconectada — nenhuma mensagem está sendo coletada destes grupos agora, mesmo
          com o monitoramento ligado.
        </div>
      )}
      ...
      <FieldHint>
        Atualizar só relê a lista de grupos do número. Não liga monitoramento nem importa mensagens
        antigas.
      </FieldHint>
      ...
      {expanded && (
        <div className="mt-3.5 border-t border-v2-track pt-3">
          <div className="mb-2 text-[11.5px] leading-snug text-v2-ink-3">
            O interruptor liga a coleta do grupo. As <b className="text-v2-ink">tags</b> não afetam
            a coleta — servem para filtrar e agrupar os sinais depois.
          </div>
    ...
    <div className="flex flex-wrap items-center gap-2.5 rounded-lg border border-v2-track px-3 py-2">
      <div className="min-w-0 flex-1">
        ...
        <div className="font-mono text-[10.5px] text-v2-faint">
          ...
          {group.is_monitored ? " · coletando" : " · fora da coleta"}
        </div>
      </div>
      ...
      <button
        ...
        aria-label={
          group.is_monitored
            ? "Parar de coletar mensagens deste grupo"
            : "Começar a coletar mensagens deste grupo"
        }
        onClick={() =>
      ...
      >
        <span
          className={`absolute top-0.5 h-4 w-4 rounded-full bg-v2-card transition-all ${

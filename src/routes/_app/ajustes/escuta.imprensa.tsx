...
export const Route = createFileRoute("/_app/ajustes/escuta/imprensa")({
  head: () => ({ meta: [{ title: "Imprensa local — Ajustes" }] }),
  component: Screen,
});
...
/**
 * Ajustes · O que eu escuto · Imprensa local (antes /ajustes/fontes, renomeada porque
 * "Fontes locais" não dizia se era fonte de notícia, de WhatsApp ou de pessoa).
 *
 * Dados reais via listVocabulary/addVocabulary/removeVocabulary (org_vocabulary,
 * kind="news_domain") — o mesmo vocabulário que alimenta as buscas de notícia em
 * src/lib/scanners.server.ts. O backend não guarda tipo (Portal/Blog/Rádio), contagem
 * de matérias 7d nem status de saúde por domínio — esses campos do mock foram
 * neutralizados em vez de inventados.
 */
function Screen() {
  ...
  return (
    <div>
      <PanelHeader
        title="Imprensa local"
        description="Domínios de portais, blogs e rádios da cidade."
        effect={
          <>
            Cada domínio aqui entra na <b className="text-v2-ink">varredura de imprensa</b> que roda
            a cada 12h. As matérias encontradas chegam em Sinais já analisadas (tema, risco,
            sentimento) e podem virar alerta. Domínio não cadastrado nunca é lido.
          </>
        }
        action={
          <div className="relative">
            ...
            {adding && (
              <div className="absolute right-0 top-11 z-50 w-72 rounded-xl border border-v2-line bg-v2-surface p-3.5 shadow-[0_16px_48px_rgba(33,31,28,0.16)]">
                ...
                />
                <FieldHint>
                  Só o domínio, sem <code>https://</code> e sem caminho. A próxima varredura já
                  passa a buscar matérias dele.
                </FieldHint>
                <button
                  onClick={submitAdd}
      ...
      <div className="mt-4 overflow-hidden rounded-[13px] border border-v2-line bg-v2-card">
        ...
        {!isLoading && domains.length === 0 && (
          <div className="px-5 py-[13px] text-[12.5px] text-v2-faint">
            Nenhuma fonte cadastrada ainda — nenhuma matéria de imprensa está entrando em Sinais.
          </div>
        )}
        {domains.map((d, i) => (
          <SourceRow
      ...
      <FieldHint>
        Remover uma fonte só interrompe as próximas varreduras. As matérias já coletadas continuam
        em Sinais e nos relatórios antigos — para apagá-las de fato, use o expurgo em Conformidade.
      </FieldHint>
  ...
  return (
    ...
    >
      ...
      <div className="flex items-center justify-between gap-2">
        <span className="font-mono text-[11px] text-v2-green">● na varredura</span>
        <button
          onClick={onRemove}
          disabled={removing}
          className="text-[11px] text-v2-faint hover:text-v2-crit disabled:opacity-50"
          aria-label={`Remover ${url} da varredura de imprensa`}
        >
          remover
        </button>
      </div>

...
export const Route = createFileRoute("/v2-eleicao")({
  head: () => ({
    meta: [{ title: "Modo Eleição — Inpol v2" }],
    links: [
      {
        rel: "stylesheet",
...
/**
 * S16 — Modo Eleição: o mesmo inPol reconfigurado para campanha.
 * Tela STANDALONE: chrome próprio de "sala de guerra" — barra dourada full-bleed no topo,
 * nav de guerra e toggle Gestão ⇄ Eleição no header. Sem o topnav padrão do cliente. Demo data.
 */
...
function Screen() {
  return (
    <div className="v2-root min-h-screen bg-v2-bg text-v2-ink">
      {/* Barra dourada full-bleed — regime de eleição */}
      <div className="flex flex-wrap items-center gap-2.5 bg-v2-panel-gold px-6 py-2 text-[12.5px] text-v2-gold-ink md:px-10">
      ...
      {/* Header de guerra — logo + nav própria + toggle Gestão/Eleição */}
      <header className="flex flex-wrap items-center gap-3.5 border-b border-v2-line bg-v2-surface px-6 py-3.5 md:px-7">
        <span className="font-display text-[21px] font-semibold text-v2-ink">
          In<i className="text-v2-green">pol</i>
          <span className="text-v2-green">.</span>
        </span>
        <nav className="flex flex-wrap items-center gap-3.5 text-[13px] text-v2-ink-3">
          {WAR_NAV.map((item) => (
            <span
              key={item}
              className={
                item === "Guerra"
                  ...
                  : "cursor-default transition-colors hover:text-v2-ink"
              }
            >
              {item}
            </span>
        ...
        <div className="flex gap-0.5 rounded-[9px] bg-v2-track p-[3px]">
          <Link
            to="/v2"
            className="rounded-[7px] px-3 py-[5px] text-[12px] font-semibold text-v2-ink-3 transition-colors hover:text-v2-ink"
          >
            Gestão
          </Link>
        ...
        <div className="relative grid h-[34px] w-[34px] place-items-center rounded-[9px] border border-v2-line bg-v2-card">
          <span aria-hidden>🔔</span>
          <span className="absolute -right-[5px] -top-[5px] rounded-lg bg-v2-crit px-[5px] py-px text-[9px] font-bold text-white">
            5
          </span>
        </div>
        <span className="grid h-8 w-8 place-items-center rounded-full bg-v2-green text-[12px] font-semibold text-white">
          MC
        </span>
      </header>

      <main className="mx-auto max-w-[1120px] px-6 py-8">
        {/* Título da sala de guerra */}
        <div className="flex flex-col gap-4 md:flex-row md:items-end md:justify-between">
          ...
          <button className="self-start rounded-[9px] bg-v2-ink px-4 py-[9px] text-[13px] font-[650] text-white md:self-auto">
            Briefing de campanha →
          </button>
        </div>

        {/* Linha 1: share of voice / termômetro / desinfo + conformidade */}

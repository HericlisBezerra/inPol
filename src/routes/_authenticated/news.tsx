  ...
  return (
    <div className="p-8 max-w-7xl space-y-6">
      <header className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
        <div>
          ...
          <p className="text-sm text-muted-foreground mt-1">
            Tudo aqui alimenta bairros, tópicos, alertas e relatórios com base no vocabulário dos ajustes.
          </p>
        </div>
        <Button size="sm" variant="outline" onClick={() => feed.refetch()} disabled={feed.isFetching} className="gap-2 w-fit">
      ...
      <section className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-5 gap-3">
        ...
        <Select value={neighborhood} onValueChange={setNeighborhood}>
          ...
          <SelectContent>
            ...
            {neighborhoods.map((v) => <SelectItem key={v.value} value={v.value}>{v.value}</SelectItem>)}
          </SelectContent>
        </Select>

        <Select value={vocabTerm} onValueChange={setVocabTerm}>
          <SelectTrigger><SelectValue placeholder="Vocabulário" /></SelectTrigger>
          <SelectContent>
            <SelectItem value="all">Todo vocabulário</SelectItem>
            {terms.map((v) => <SelectItem key={`${v.kind}:${v.value}`} value={v.value}>{v.value}</SelectItem>)}
          </SelectContent>
        </Select>
      ...
      <section className="space-y-3">
        {feed.isLoading && <Card className="p-6 bg-surface text-sm text-muted-foreground">Carregando sinais…</Card>}
        {feed.data?.length === 0 && <Card className="p-6 bg-surface text-sm text-muted-foreground">Nenhum sinal encontrado com esses filtros.</Card>}
        {feed.data?.map((row) => {
          ...
          const title = payload?.title ?? analysis?.summary ?? String(row.content ?? "").slice(0, 120);
              ...
              <div className="flex flex-col gap-3 md:flex-row md:items-start md:justify-between">
                <div className="min-w-0 space-y-2">
                  <div className="flex flex-wrap items-center gap-2 text-xs text-muted-foreground">
                    <Badge variant="outline" className="gap-1"><Newspaper className="size-3" /> {SOURCE_LABEL[sourceData?.kind ?? ""] ?? sourceData?.kind ?? "fonte"}</Badge>
                    <span className="font-mono">{new Date(row.posted_at).toLocaleString("pt-BR")}</span>
                    {group?.subject && <span>· {group.subject}</span>}
                    {(analysis?.neighborhood ?? group?.neighborhood_tag) && <span>· {(analysis?.neighborhood ?? group?.neighborhood_tag)}</span>}
                    {analysis?.topic && <span>· {analysis.topic}</span>}
                  </div>
                  {payload?.url ? (
                    <a href={payload.url} target="_blank" rel="noopener noreferrer" className="font-medium hover:text-primary inline-flex items-center gap-1">
                      {title} <ExternalLink className="size-3" />
                    </a>
                  ) : (
                    <h2 className="font-medium">{title}</h2>
                  )}
                  <p className="text-sm text-muted-foreground line-clamp-3">{analysis?.summary ?? row.content}</p>
                  <div className="flex flex-wrap gap-1.5">
                    {[...(analysis?.mentioned_opponents ?? []), ...(analysis?.mentioned_entities ?? []), ...(analysis?.mentioned_allies ?? [])].slice(0, 8).map((term) => (
                      <Badge key={term} variant="secondary">{term}</Badge>
                    ))}
                  </div>
                </div>
                <div className="md:text-right shrink-0">
                  <div className={`font-display text-3xl ${risk >= 70 ? "text-destructive" : risk >= 45 ? "text-warning" : "text-muted-foreground"}`}>{risk}</div>
                  <div className="label-mono text-[10px]">risco</div>
                  <div className="text-xs text-muted-foreground mt-1">sent. {Number(analysis?.sentiment ?? 0).toFixed(2)}</div>
                </div>
              </div>
            </Card>
          );

...
function ReportDetail() {
  ...
  const topTopics = (data.top_topics ?? []).map((topic, index) => ({
    ...topic,
  ...
  return (
    <div className="p-8 max-w-5xl space-y-6">
      ...
      {/* Hero */}
      <Card
        className={`relative overflow-hidden p-8 bg-gradient-to-br ${meta.gradient} border-primary/20`}
      >
        <div className="absolute -top-10 -right-10 text-[180px] opacity-20 select-none">
          {meta.emoji}
        </div>
        <div className="relative">
          ...
          <div className="text-sm text-muted-foreground mt-2 font-mono">
            🗓️ {new Date(report.period_start).toLocaleDateString("pt-BR")} →{" "}
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        <KpiCard
          ...
          label="Mensagens"
          value={(data.counts?.messages_analyzed ?? 0).toLocaleString("pt-BR")}
          delta={`${sourceCounts.news ?? 0} portais · ${sourceCounts.instagram ?? 0} Instagram`}
          tone="up"
        />
        <KpiCard
          emoji="🚨"
          ...
          label="Alertas"
          value={String(alertPie.reduce((sum, item) => sum + item.value, 0))}
          delta={`${alertPie[0].value} críticos`}
          tone="warn"
        />
        <KpiCard
          emoji="🔥"
          ...
          delta={hottest?.label ?? "sem tema dominante"}
          tone="up"
        />
        <KpiCard
          emoji="📍"
          ...
          value={String(data.top_neighborhoods?.length ?? 0)}
          delta={topNeighborhood?.label ?? "sem bairro detectado"}
          tone="neutral"
        />
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
        <Card className="p-5 bg-surface lg:col-span-2">
          ...
          <p className="text-xs text-muted-foreground mb-3">
            média diária — 0 é neutro, &gt;0 positivo
          </p>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
        <HighlightCard
          ...
          body={
            hottest
              ? `${hottest.label} · risco máx ${hottest.max_risk ?? 0} · ${hottest.count} menções.`
              : "Sem risco relevante no período."
          }
        />
        <HighlightCard
          tone="good"
          emoji="🌟"
          title="Bairro mais ativo"
          body={
            topNeighborhood
              ? `${topNeighborhood.label} · ${topNeighborhood.count} sinais · sentimento ${topNeighborhood.avg_sentiment}.`
              : "Nenhum bairro detectado pelo vocabulário."
          }
        />
        <HighlightCard
          tone="watch"
          ...
          body={
            topExternal
              ? `${topExternal.source}: ${topExternal.title ?? topExternal.summary}`
              : "Sem sinal externo analisado no período."
          }
        />
      </div>
      ...
      <Card className="p-6 bg-surface">
        <div className="flex items-center gap-2 mb-3">
          ...
          <span className="text-xs text-muted-foreground ml-2">
            expanda para ver os sinais brutos que alimentaram este relatório
          </span>
        </div>
        <Accordion type="multiple" className="w-full">
          <AccordionItem value="topics">
            <AccordionTrigger className="text-sm">
              🔥 Temas com citações reais ({(data.top_topics ?? []).length})
            </AccordionTrigger>
            <AccordionContent>
                  ...
                  <div key={topic.label} className="border-l-2 border-primary/40 pl-3">
                    <div className="flex items-baseline gap-2 flex-wrap">
                      ...
                      <span className="text-xs text-muted-foreground font-mono">
                        {topic.count} menções · sent {topic.avg_sentiment ?? 0} · risco máx{" "}
                        {topic.max_risk ?? 0}
                      </span>
                    </div>
                      ...
                      <ul className="mt-2 space-y-1.5 text-xs">
                        {(topic.samples ?? []).map((s, i) => (
                          <li key={i} className="text-muted-foreground">
                            <span className="text-[10px] uppercase font-mono mr-2 opacity-70">
                              {s.source}
          ...
          <AccordionItem value="external">
            <AccordionTrigger className="text-sm">
              📰 Sinais externos ({(data.external_signals ?? []).length})
            </AccordionTrigger>
            <AccordionContent>
              <div className="space-y-3">
                {(data.external_signals ?? []).map((s, i) => (
                  <div key={i} className="text-xs border-b border-border/50 pb-2 last:border-0">
                    ...
                    {s.excerpt && s.excerpt !== s.summary && (
                      <p className="italic text-muted-foreground/80 mt-1">"{s.excerpt}"</p>
                    )}
                  </div>
                ))}
                {(data.external_signals ?? []).length === 0 && (
                  <p className="text-xs text-muted-foreground">
                    Nenhum sinal externo capturado no período.
                  </p>
                )}
              </div>
            </AccordionContent>
          ...
          <AccordionItem value="highrisk">
            <AccordionTrigger className="text-sm">
              ⚠️ Mensagens de maior risco ({(data.high_risk_messages ?? []).length})
            </AccordionTrigger>
            <AccordionContent>
              <div className="space-y-2">
                ...
                {(data.high_risk_messages ?? []).length === 0 && (
                  <p className="text-xs text-muted-foreground">
                    Nenhuma mensagem com risco elevado no período.
                  </p>
                )}
              </div>
            </AccordionContent>
          ...
          <AccordionItem value="neighborhoods">
            <AccordionTrigger className="text-sm">
              📍 Bairros × temas ({(data.top_neighborhoods ?? []).length})
            </AccordionTrigger>
            <AccordionContent>
          ...
          <AccordionItem value="alerts">
            <AccordionTrigger className="text-sm">
              🚨 Alertas do período ({(data.sample_alerts ?? []).length})
            </AccordionTrigger>
            <AccordionContent>
              <div className="space-y-2">
                    ...
                    <div key={i} className="text-xs border-l-2 border-amber-500/40 pl-3">
                      ...
                      {a.recommended_action && (
                        <p className="text-foreground/80 mt-0.5">
                          <strong>Ação:</strong> {a.recommended_action}
                        </p>
                      )}
                    </div>
                  );
                })}
                {(data.sample_alerts ?? []).length === 0 && (
                  <p className="text-xs text-muted-foreground">
                    Nenhum alerta disparado no período.
                  </p>
                )}
              </div>
            </AccordionContent>
      ...
      <div className="text-xs text-muted-foreground text-center py-4">
        🤖 Gerado por Inpol · modelo {report.model_version} ·{" "}

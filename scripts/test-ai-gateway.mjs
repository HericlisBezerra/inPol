...
let threw = false;
try { await callAi({ messages: [{ role: "user", content: "x" }], maxAttempts: 3 }); } catch { threw = true; }
...
let urls = [];
global.fetch = async (u) => { urls.push(u); return res(200, okBody("via-gemini")); };
...
ok(urls.length === 1 && urls[0].includes("generativelanguage"), "não faz fetch no provedor sem chave");

// 5) BAD deepseek key (401) -> falls to gemini fallback (o cenário do usuário)
process.env.DEEPSEEK_API_KEY = "bad-key";
urls = [];
global.fetch = async (u) => { urls.push(u); return u.includes("deepseek") ? res(401, {}) : res(200, okBody("salvo-pelo-gemini")); };
const r5 = await callAi({ model: "deepseek-chat", fallbackModels: ["gemini-3.5-flash"], messages: [{ role: "user", content: "x" }] });
ok(r5.text === "salvo-pelo-gemini" && r5.model === "gemini-3.5-flash", "chave DeepSeek ruim (401) cai no Gemini");
delete process.env.DEEPSEEK_API_KEY;
